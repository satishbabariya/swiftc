...
///
  ...
  public:
    Token(tok Kind, llvm::StringRef Text, unsigned CommentLength = 0)
...
#include "TokenKinds.def"
    ...
    SourceLocation getLoc() const {
      return SourceLocation(llvm::SMLoc::getFromPointer(Text.begin()));
    }
    ...
    CharSourceRange getCommentRange() const {
      if (CommentLength == 0)
        return CharSourceRange(SourceLocation(llvm::SMLoc::getFromPointer(Text.begin())),
                               0);
      auto TrimedComment = trimComment();
      return CharSourceRange(
        SourceLocation(llvm::SMLoc::getFromPointer(TrimedComment.begin())),
        TrimedComment.size());
    }

    SourceLocation getCommentStart() const {
      if (CommentLength == 0) return SourceLocation();
      return SourceLocation(llvm::SMLoc::getFromPointer(trimComment().begin()));
    }
